...
packageRouter.patch('/transport-requests/:id/status', authorize(...OPERATIONS_ROLES), async (req, res, next) => {
  try {
    const status = String(req.body.status || ''); const adminNote = String(req.body.adminNote || '').trim();
    if (!['PENDING', 'APPROVED', 'PICKING_UP', 'IN_TRANSIT', 'COMPLETED', 'CANCELLED'].includes(status)) return res.status(400).json({ success: false, message: 'Trạng thái yêu cầu vận chuyển không hợp lệ' });
    ...
    const data = await prisma.transportRequest.update({ where: { id: existing.id }, data: { status, adminNote: adminNote || existing.adminNote } });
    ...
    const transportLabel = transportLabels[status] || `Trạng thái: ${status}`;
    await prisma.notification.create({ data: { userId: existing.customerId, title: `Yêu cầu vận chuyển ${existing.requestCode} - ${transportLabel}`, message: adminNote || transportLabel, type: 'TRANSPORT_REQUEST', targetRole: 'CUSTOMER' } });
    const customer = await prisma.user.findUnique({ where: { id: existing.customerId } });
    if (customer?.email) {
      void EmailService.notifyCustomerAlert({
        toEmail: customer.email,
